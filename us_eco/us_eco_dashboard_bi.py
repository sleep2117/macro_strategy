...
def _init_bi_state(fields: list[str], numeric_fields: list[str]) -> None:
    if not fields:
        return
    if "bi_x" not in st.session_state or st.session_state["bi_x"] not in fields:
        st.session_state["bi_x"] = "date" if "date" in fields else fields[0]
    if "bi_y" not in st.session_state or st.session_state["bi_y"] not in numeric_fields:
        st.session_state["bi_y"] = "value" if "value" in numeric_fields else numeric_fields[0]
    if "bi_color" not in st.session_state or st.session_state["bi_color"] not in fields:
        st.session_state["bi_color"] = "display_label" if "display_label" in fields else "series_label"
    if "bi_size" not in st.session_state or st.session_state["bi_size"] not in numeric_fields:
        st.session_state["bi_size"] = ""
    if "bi_facet_row" not in st.session_state or st.session_state["bi_facet_row"] not in fields:
        st.session_state["bi_facet_row"] = ""
    if "bi_facet_col" not in st.session_state or st.session_state["bi_facet_col"] not in fields:
        st.session_state["bi_facet_col"] = ""


def render_bi_dashboard(metas: list[dict[str, Any]]) -> None:
    registry, _, default_checked = build_series_registry(metas)
    if not registry:
        st.warning("No series available for BI builder.")
        return

    module_map: dict[str, str] = {}
    for info in registry.values():
        module_map.setdefault(info["module_label"], info["stem"])
    module_labels = sorted(module_map.keys())
    default_module_label = None
    for label, stem in module_map.items():
        if stem == DEFAULT_MODULE_FOR_SELECTION:
            default_module_label = label
            break
    if default_module_label is None and module_labels:
        default_module_label = module_labels[0]
    ...
    with st.sidebar:
        ...
        selected_modules = st.multiselect(
            ...
            default=[default_module_label] if default_module_label else [],
            key="bi_modules",
        )
        selected_stems = {module_map[label] for label in selected_modules if label in module_map}
        series_search = st.text_input("Series search", value="", key="bi_series_search")

        series_candidates = [info for info in registry.values() if info["stem"] in selected_stems]
        if series_search:
            needle = series_search.strip().lower()
            series_candidates = [
                info for info in series_candidates
                if needle in info["display_label"].lower()
            ]

        label_to_key = {info["display_label"]: info["key"] for info in series_candidates}
        series_labels = sorted(label_to_key.keys())
        default_labels = [
            registry[key]["display_label"]
            for key in default_checked
            if key in registry and registry[key]["stem"] in selected_stems
        ]

        selected_labels = st.multiselect(
            "Series",
            options=series_labels,
            default=[label for label in default_labels if label in series_labels],
            key="bi_series",
        )

        dtype_label_map = {label: key for key, label in STANDARD_DATA_KEYS}
        dtype_labels = [label for _, label in STANDARD_DATA_KEYS]
        ...
        dtype_key = dtype_label_map[selected_dtype_label]

        months_back = st.slider("Lookback (months)", 6, 120, 60, key="bi_lookback")

    if not selected_labels:
        st.info("Select one or more series to start.")
        return

    selected_keys = [label_to_key[label] for label in selected_labels if label in label_to_key]
    selected_infos = [registry[key] for key in selected_keys if key in registry]
    ...
    if df_long.empty:
        st.warning("No data after applying lookback filter.")
        return

    filter_cols = ["module", "category", "series_label", "data_type"]
    with st.sidebar.expander("Filters", expanded=False):
        for col in filter_cols:
            if col not in df_long.columns:
                continue
            options = sorted([item for item in df_long[col].dropna().unique()])
            if not options:
                continue
            selected = st.multiselect(
                ...
                options=options,
                default=options,
                key=f"bi_filter_{col}",
            )
            if selected and len(selected) != len(options):
                df_long = df_long[df_long[col].isin(selected)]
    ...
    field_pool = [
        col for col in ["date", "value", "series_label", "display_label", "module", "category", "data_type"]
        if col in df_long.columns
    ]
    numeric_fields = [col for col in df_long.select_dtypes(include="number").columns if col in field_pool]

    _init_bi_state(field_pool, numeric_fields)

    st.subheader("BI Builder")
    left, right = st.columns([1, 2])

    with left:
        st.markdown("#### Field Panel")
        st.caption("Pick a field and assign it to X/Y/Color/Size/Facet.")
        st.dataframe(
            pd.DataFrame({"Fields": field_pool}),
            use_container_width=True,
            hide_index=True,
        )
        selected_field = st.selectbox("Field", options=field_pool, key="bi_field_picker")
        assign_col1, assign_col2, assign_col3 = st.columns(3)
        with assign_col1:
            st.button("Drop to X", on_click=lambda: st.session_state.__setitem__("bi_x", selected_field))
            st.button("Drop to Y", on_click=lambda: st.session_state.__setitem__("bi_y", selected_field))
        with assign_col2:
            st.button("Drop to Color", on_click=lambda: st.session_state.__setitem__("bi_color", selected_field))
            st.button("Drop to Size", on_click=lambda: st.session_state.__setitem__("bi_size", selected_field))
        with assign_col3:
            st.button("Facet Row", on_click=lambda: st.session_state.__setitem__("bi_facet_row", selected_field))
            st.button("Facet Col", on_click=lambda: st.session_state.__setitem__("bi_facet_col", selected_field))

    with right:
        st.markdown("#### Drop Zones")
        x_field = st.selectbox("X", options=field_pool, index=field_pool.index(st.session_state["bi_x"]), key="bi_x_select")
        y_options = numeric_fields if numeric_fields else field_pool
        y_default = y_options.index(st.session_state["bi_y"]) if st.session_state["bi_y"] in y_options else 0
        y_field = st.selectbox("Y", options=y_options, index=y_default, key="bi_y_select")
        color_options = [""] + field_pool
        color_default = color_options.index(st.session_state["bi_color"]) if st.session_state["bi_color"] in color_options else 0
        color_field = st.selectbox("Color", options=color_options, index=color_default, key="bi_color_select")
        size_options = [""] + numeric_fields
        size_default = size_options.index(st.session_state["bi_size"]) if st.session_state["bi_size"] in size_options else 0
        size_field = st.selectbox("Size", options=size_options, index=size_default, key="bi_size_select")
        facet_row_options = [""] + field_pool
        facet_row_default = facet_row_options.index(st.session_state["bi_facet_row"]) if st.session_state["bi_facet_row"] in facet_row_options else 0
        facet_row = st.selectbox("Facet Row", options=facet_row_options, index=facet_row_default, key="bi_facet_row_select")
        facet_col_options = [""] + field_pool
        facet_col_default = facet_col_options.index(st.session_state["bi_facet_col"]) if st.session_state["bi_facet_col"] in facet_col_options else 0
        facet_col = st.selectbox("Facet Col", options=facet_col_options, index=facet_col_default, key="bi_facet_col_select")

        st.session_state["bi_x"] = x_field
        st.session_state["bi_y"] = y_field
        st.session_state["bi_color"] = color_field
        st.session_state["bi_size"] = size_field
        st.session_state["bi_facet_row"] = facet_row
        st.session_state["bi_facet_col"] = facet_col

        chart_type = st.selectbox(
            "Chart",
            options=["line", "bar", "scatter", "area", "box"],
            index=0,
            key="bi_chart_type",
        )
        agg_method = st.selectbox(
            "Aggregation",
            options=["none", "mean", "sum", "median", "last"],
            index=0,
            key="bi_agg",
        )
        ...
        fig = None
        if chart_type == "line":
            fig = px.line(df_plot, x=x_field, y=y_field, color=color_field or None, facet_row=facet_row or None, facet_col=facet_col or None)
        elif chart_type == "bar":
            fig = px.bar(df_plot, x=x_field, y=y_field, color=color_field or None, facet_row=facet_row or None, facet_col=facet_col or None, barmode="group")
        elif chart_type == "scatter":
            fig = px.scatter(df_plot, x=x_field, y=y_field, color=color_field or None, size=size_field or None, facet_row=facet_row or None, facet_col=facet_col or None)
        elif chart_type == "area":
            fig = px.area(df_plot, x=x_field, y=y_field, color=color_field or None, facet_row=facet_row or None, facet_col=facet_col or None)
        elif chart_type == "box":
            fig = px.box(df_plot, x=x_field, y=y_field, color=color_field or None, facet_row=facet_row or None, facet_col=facet_col or None)
        ...
        with st.expander("Data Preview", expanded=False):
            ...
            csv_bytes = df_plot.to_csv(index=False).encode("utf-8-sig")
            st.download_button(
                label="Download CSV",
                data=csv_bytes,
                file_name=f"bi_export_{datetime.now().strftime('%Y%m%d_%H%M')}.csv",
                mime="text/csv",
            )
...
def main():
    ...
    mode = st.sidebar.radio(
        "Dashboard Mode",
        options=["BI Builder", "Classic"],
        index=0,
        key="bi_mode",
    )
    if mode == "BI Builder":
        render_bi_dashboard(metas)
    else:
